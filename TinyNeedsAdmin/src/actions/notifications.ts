'use server';

import { createClient } from '@/supabase/server';

async function sendPushNotification({
  expoPushToken,
  title,
  body,
}: {
  expoPushToken: string;
  title: string;
  body: string;
}) {
  const message = {
    to: expoPushToken,
    sound: 'default',
    title,
    body,
    data: { someData: 'goes here' },
  };

  await fetch('https://exp.host/--/api/v2/push/send', {
    method: 'POST',
    headers: {
      Accept: 'application/json',
      'Accept-encoding': 'gzip, deflate',
      'Content-Type': 'application/json',
    },
    body: JSON.stringify(message),
  });
}

export const getUserNotificationToken = async (userId: string) => {
  const supabase = createClient();
  
  try {
    const { data, error } = await supabase
      .from('users')
      .select('expo_notification_token')
      .eq('id', userId)
      .single();

    if (error) {
      // If the column doesn't exist, return null instead of throwing an error
      if (error.message.includes('expo_notification_token') && error.message.includes('does not exist')) {
        console.warn('expo_notification_token column does not exist in users table. Please run the migration to add this column.');
        return { expo_notification_token: null };
      }
      throw new Error(error.message);
    }

    return data;
  } catch (error) {
    console.error('Error fetching notification token:', error);
    return { expo_notification_token: null };
  }
};

export const sendNotification = async (userId: string, status: string) => {
  try {
    const tokenData = await getUserNotificationToken(userId);

    if (!tokenData?.expo_notification_token) {
      console.log('No notification token found for user:', userId);
      return;
    }

    await sendPushNotification({
      expoPushToken: tokenData.expo_notification_token,
      title: 'Your Order Status',
      body: `Your order is now ${status}`,
    });
  } catch (error) {
    console.error('Error sending notification:', error);
  }
};
